...
export async function loadData():Promise<Dataset> { if(!db) return demo(); const names:CollectionName[]=['users','versions','prs','issues','regressionTests','knowledgeArticles']; const entries=await Promise.all(names.map(async name=>[name,(await getDocs(collection(db!,name))).docs.map(d=>({ ...d.data(),id:d.id }))] as const)); return Object.fromEntries(entries) as Dataset; }
